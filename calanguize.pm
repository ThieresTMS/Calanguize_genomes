...
sub read_config_files { #read config files in the form element = value #comment
  ...
  open(my $fh_calanguize_config, "<", "$calanguize_path/calanguize_config") || die ("The configuration file 'potion_config' couldn't be read, please check if the file exists and if its permissions are properly set.\n");
...
sub check_parameters { #check for all parameters, 
  ...
  #if (!-w $parameters->{outdir}) { die ("You don't have permission to write in the Calanguize directory, please redefine your permissions for this directory.\n"); }
  ...
  # -=-=-= PATH TO EXTERNAL PROGRAMS USED BY CALANGUIZE =-=-=-
  if (!defined $parameters->{python_path}) { die ("No path to Python was specified in calanguize_config at $config_path, please open this file and fill the parameter 'Python3.6'.\n"); }
  ...
  if (!-x $parameters->{python_path}) { die ("You don't have permission to execute the Python file specified at calanguize_config, please check permissions or replace the file\n"); }

 if (!defined $parameters->{busco_path}) { die ("No path to BUSCO was specified in calanguize_config at $config_path, please open this file and fill the parameter 'busco'.\n"); }
  ...
  if (!-x $parameters->{busco_path}) { die ("You don't have permission to execute the BUSCO file specified at calanguize_config, please check permissions or replace the file\n"); }

  if (!defined $parameters->{interpro_path}) { die ("No path to INTERPRO was specified in calanguize_config at $config_path, please open this file and fill the parameter 'interpro'.\n"); }
  ...
  if (!-x $parameters->{interpro_path}) { die ("You don't have permission to execute the INTERPRO file specified at calanguize_config, please check permissions or replace the file\n"); }
...
sub create_directory_structure {
  ...
  if (!-e "$root_outdir/calango_imputs") {mkdir ("$root_outdir/calango_imputs") || die ("Couldn't create the directory specified as '$root_outdir', check if you are trying to create a subdirectory in a non-existent directory or if you don't have permission to create a directory there.\n");
    mkdir ("$root_outdir/calango_imputs/feature2GO");
    mkdir ("$root_outdir/calango_imputs/gene2GO");
    mkdir ("$root_outdir/calango_imputs/pfam");
  }
}
...
sub get_genomes { 
  ...
  my @ids_err;
  open (ID_ERR, ">", "$root_outdir/species_not_found.txt") or die ("");
...
sub gene2GO{
  ...
  #output directory
  my $outdir_path = "$root_outdir/calango_imputs/gene2GO";
  chomp $outdir_path;
...
sub feature2GO{
  ...
  my @path = split(/\//, $in);
  my $outdir_path = "$root_outdir/calango_imputs/feature2GO";
...
sub parser_pfam {
  ...
  #output directory
  my $outdir_path = "$root_outdir/calango_imputs/pfam";
  chomp $outdir_path;
