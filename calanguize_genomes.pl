...
#read config files
my $calanguize_path = dirname (rel2abs($0));
my $parameters = read_config_files ($project_config, $calanguize_path);
...
print $log_fh ("Welcome to Calanguize Genomes\n");
...
#creating CALANGO-compatible output files
GENE2GO:
foreach my $id (@ids) {
  my $pid = $pm ->start and next GENE2GO;
  print "-> Creat calango compatible imput GENE to Gene Ontology\n";
...
foreach my $id (@ids) {
  ...
  print "-> Creat calango compatible imput Pfam to Gene Ontology\n";
...
foreach my $id (@ids) {
  ...
  print "-> Creat calango compatible imput Pfam\n";
...
#  $pm -> wait_all_children;
#}
print "Finish! Closing Calanguize, see you around\n";
